...
// Where each X lands. Capitals mostly — but Ottawa and Washington sit close
// enough to collide at this scale, so Canada is marked at its centre instead.
const CAP = {
  Canada: [-95.0, 57.0],
...
// Label position relative to its X. Canada's sits above so the word doesn't
// run into the United States mark below it.
const LABEL_DY = { Canada: -24 };
const DEFAULT_LABEL_DY = 34;
...
const world = (() => {
  ...
  const marks = WORLD_MARKS.map((name) => {
    ...
    return {
      ...
      labelDy: LABEL_DY[name] ?? DEFAULT_LABEL_DY,
...
const out =
  ...
  `export type WorldMark = { name: string; x: number; y: number; label: boolean; labelDy: number; europe: boolean; pending: boolean };\n` +
